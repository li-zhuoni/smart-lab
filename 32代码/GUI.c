...
void Gui_DrawFont_GBK25(uint16_t x, uint16_t y, uint16_t fc, uint16_t bc, uint8_t *s)
{
	unsigned char i,j;
	unsigned short k;
	...
	while(*s) 
	{
		...
		else 
		{
			...
			for (k=0;k<hz25_num;k++) 
			{
			  if ((hz25[k].Index[0]==*(s))&&(hz25[k].Index[1]==*(s+1)))
			  { 
				    for(i=0;i<24;i++)
				    {
						for(j=0;j<24;j++) 
							{
						    	if(hz25[k].Msk[i*3]&(0x80>>j))
								Gui_DrawPoint(x+j,y+i,fc);
								else 
								{
									if (fc!=bc) Gui_DrawPoint(x+j,y+i,bc);
						...
						for(j=0;j<24;j++) 
							{
						    	if(hz24[k].Msk[i*3+1]&(0x80>>j))	Gui_DrawPoint(x+j+24,y+i,fc);
								else {
